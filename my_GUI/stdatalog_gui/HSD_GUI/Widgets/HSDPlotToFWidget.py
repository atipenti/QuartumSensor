...
class HSDPlotToFWidget(PlotWidget):    
    def __init__(self, controller, comp_name, comp_display_name, plot_params, p_id=0, parent=None):
        ...
        try:
            ...
            if os.path.exists(config_path):
                ...
                # Php 20260301 - 100% Dynamic configuration lookup
                sensors_dict = self.app_cfg.get("network_config", {}).get("sensors", {})
                
                # We look for settings using self.comp_name (the technical ID from device_config)
                # Fallback to the uppercase version or a generic empty dict if not found
                self.sensor_settings = sensors_dict.get(self.comp_name, 
                                       sensors_dict.get(self.comp_name.upper(), {}))
                
            else:
                print(f"[DEBUG] Config NOT FOUND at {config_path}. Using defaults.")
                self.app_cfg = {"network_config": {"sensors": {}}}
        ...
        # Php 20260301 --- 2. INITIALIZE BROADCASTER ---
        if UDPBroadcaster:
        ...
        self.t1_out.set_default_rotation(self.sensor_settings.get("rotation", 0))
        self.t1_out.set_default_x_flip(self.sensor_settings.get("flip_x", False))
        self.t1_out.set_default_y_flip(self.sensor_settings.get("flip_y", False))
        
        # 3. VISUAL SYNC: Update the GUI buttons/labels to match the config
        settings = self.t1_out.rois_frame 
        settings.flipped_x_status = self.sensor_settings.get("flip_x", False)
        settings.flipped_y_status = self.sensor_settings.get("flip_y", False)
        
        # Update the button text based on the config labels
        settings.flip_x_label.setText(self.sensor_settings.get("label_x", "Normal"))
        settings.flip_y_label.setText(self.sensor_settings.get("label_y", "Normal"))
    ...
    def add_data(self, data):
        ...
        try:
            ...
            # Php 20260228 without UDP boradcaster --- UDP STREAMING ---
            ...
            # Php 20260301 --- MODULAR UDP FORWARDING ---
            # if t1_data is not None:
            #    self.broadcaster.send_sensor_data("VL53L8_ToF", t1_data)
            # --- To this ---
            # Php 20260301 --- MODULAR UDP FORWARDING ---
            # No hard-coded strings for sensor name "VL53L8_ToF" here anymore!
            if self.broadcaster and t1_data is not None:
                self.broadcaster.send_sensor_data(self.comp_name, t1_data)
