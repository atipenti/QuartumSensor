...
from stdatalog_gui.Utils.PlotParams import PlotLabelParams, PlotLevelParams, PlotGaugeParams, PlotCheckBoxParams
...
class PlotsArrayWidget(PlotWidget):
    def __init__(self, controller, comp_name, comp_display_name, out_plots, p_id=0, parent=None, left_label=None, orientation="h"):
        ...
        self.plot_widget = self.loader.load(os.path.join(os.path.dirname(stdatalog_gui.__file__),"UI","plots_array_output.ui"), parent)
...
class SubPlotFrame(QWidget):
    def __init__(self, pw, sp_id, parent_wdgt:PlotsArrayWidget):
        ...
        sub_plot_ui_widget = self.parent_wdgt.loader.load(os.path.join(os.path.dirname(stdatalog_gui.__file__),"UI","sub_plot.ui"), self.parent_wdgt)
